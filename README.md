...
This release supports inventory and lifecycle operations for AWS EC2, Alibaba Cloud ECS, Azure Virtual Machines and Google Compute Engine; it does not provision or terminate those compute instances. Every compute, Alibaba EDS, and Alibaba OSS listing page reads the latest PostgreSQL snapshot first and queues live collection asynchronously, so provider latency does not block navigation. EDS Users consumes one all-regions desktop snapshot instead of issuing one browser request per region. OSS metadata and usage reads use bounded per-bucket concurrency; provider-reported usage can lag by more than one hour. Successful results replace the prior snapshot while provider failures retain the last healthy data. Equivalent jobs are coalesced, account/service leases prevent overlapping collection, accounts and regions use bounded concurrency, inventory rows are batch-upserted, and Azure VM status reads are parallelized with a fixed limit. Operations that can create charges require explicit confirmation, and AutoPay defaults to off. AWS and Alibaba collection is regional; Azure and GCP use subscription/project-wide inventory endpoints with optional region filtering. Mutation intents must be stored before an action is submitted; other audit writes are best effort and failures are logged. Versioned SQL migrations run transactionally on startup. SAML supports SP-initiated login, optional IdP-initiated login, and signed Single Logout responses; OIDC uses discovery and RP-initiated logout when the provider publishes an end-session endpoint.
